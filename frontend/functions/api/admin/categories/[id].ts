...
export async function onRequestPatch({ request, env, params }: PagesContext) {
  ...
  const body = await readJson<{ name?: string; active?: boolean; sortOrder?: number; gridColumns?: number }>(request);
  const name = cleanText(body.name, 80);
  if (!name) return fail('分类名称不能为空');
  const gridColumns = Number(body.gridColumns) === 1 ? 1 : 2;
  await env.DB.prepare('UPDATE categories SET name = ?, active = ?, sort_order = ?, grid_columns = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
    .bind(name, body.active === false ? 0 : 1, Number(body.sortOrder) || 0, gridColumns, params.id).run();
  return json({ ok: true });
}
