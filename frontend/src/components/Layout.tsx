...
export default function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className={`min-h-screen flex transition-colors duration-200 relative ${isDark ? 'bg-black' : ''
      }`}
      style={isDark ? {} : {
      ...
      {isMobile && (
        <button
          ...
          className={`fixed top-4 left-4 z-20 p-2 rounded-md active:scale-95 outline-none focus-visible:ring-2 focus-visible:ring-gray-400/40 ${isDark ? 'bg-black text-white' : 'glass-light text-black'
            } shadow-md`}
        >
          {isSidebarOpen ? '✕' : '☰'}
        </button>
      ...
      <div className={`
        ...
        ${isDark ? 'bg-black border-gray-700' : 'bg-transparent border-gray-200'}
        border-r p-6
      `}>
        {isSidebarOpen && (
          <>
            ...
            <div className="mb-6 flex justify-between items-center">
              ...
              <button
                ...
                className={`w-12 h-6 rounded-full p-1 transition-colors duration-200 flex items-center ${isDark ? 'bg-gray-600' : 'bg-gray-300'
                  }`}
              >
