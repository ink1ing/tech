...
import { ExternalLink, Image, ImagePlus as Images, LayoutGrid, LogOut, Moon, PackagePlus, RefreshCw, Save, Search, ShieldCheck, Sun, Trash2, Upload } from 'lucide-react';
import { storeApi } from '../services/storeApi';
import type { Category, OrderSummary, Product, ProductImage } from '../types/store';
import '../styles/store.css';

const emptyProduct: Product = { id: '', category_id: '', slug: '', name: '', subtitle: '', description: '', price_cents: 0, original_price_cents: 0, image_id: '', image_url: '', description_images: [], fulfillment: 'digital', delivery_note: '', icon: 'Package', sort_order: 0, active: 1 };
...
export default function StoreAdminPage() {
  ...
  const [darkMode, setDarkMode] = useState(() => localStorage.getItem('silas-store-admin-theme') === 'dark');
  ...
  useEffect(() => { localStorage.setItem('silas-store-admin-theme', darkMode ? 'dark' : 'light'); }, [darkMode]);
  ...
  if (!token) return <div className="store-admin admin-login" data-theme={darkMode ? 'dark' : 'light'}><button className="admin-theme-toggle" title="切换深色模式" onClick={() => setDarkMode(value => !value)}>{darkMode ? <Sun /> : <Moon />}</button><form onSubmit={login}><ShieldCheck size={36} /><h1>商店管理</h1><p>输入 Cloudflare 中配置的管理员密码。</p><label className="admin-field">管理员密码<input type="password" autoComplete="current-password" required value={password} onChange={event => setPassword(event.target.value)} /></label>{error && <p className="admin-error">{error}</p>}<button className="store-primary" disabled={busy}>{busy ? '正在登录…' : '登录'}</button></form></div>;

  return <div className="store-admin admin-shell" data-theme={darkMode ? 'dark' : 'light'}>
    ...
    <main className="admin-main">
      <header className="admin-header"><div><small>管理员后台</small><h2>{tab === 'orders' ? '订单管理' : tab === 'products' ? '商品管理' : tab === 'categories' ? '分类管理' : '页面设置'}</h2></div><div className="admin-toolbar">{tab === 'orders' && <input value={search} onChange={event => setSearch(event.target.value)} placeholder="搜索订单" />}<button className="admin-secondary admin-theme-button" title="切换深色模式" onClick={() => setDarkMode(value => !value)}>{darkMode ? <Sun size={16} /> : <Moon size={16} />}</button><button className="admin-secondary" onClick={loadData} disabled={busy}><RefreshCw size={15} /> 刷新</button>{tab === 'products' && <button className="store-primary" onClick={() => setEditingProduct({ ...emptyProduct, category_id: categories[0]?.id || '' })}><PackagePlus size={15} /> 新增商品</button>}</div></header>
      ...
      {tab === 'products' && <>
        <div className="admin-product-filters"><label><Search size={15} /><input value={productSearch} onChange={event => setProductSearch(event.target.value)} placeholder="搜索商品" /></label><select value={productCategory} onChange={event => setProductCategory(event.target.value)}><option value="all">全部分类</option>{categories.map(category => <option key={category.id} value={category.id}>{category.name}</option>)}</select><div className="segmented">{[['all','全部'],['digital','非邮寄'],['shipping','需邮寄']].map(([value,label]) => <button type="button" key={value} className={productFulfillment === value ? 'active' : ''} onClick={() => setProductFulfillment(value)}>{label}</button>)}</div></div>
        <div className="admin-product-grid">{filteredProducts.map(product => { const hasDiscount = product.original_price_cents > product.price_cents; const discount = hasDiscount ? Number(((product.price_cents / product.original_price_cents) * 10).toFixed(1)) : null; return <article className="admin-product-row" key={product.id}>{product.image_url && <img className="admin-product-thumb" src={product.image_url} alt="" />}<h3>{product.name}</h3><p>{product.category_name} · ¥{(product.price_cents / 100).toFixed(2)}{hasDiscount && <> / 原价 ¥{(product.original_price_cents / 100).toFixed(2)} · {discount}折</>} · {product.fulfillment === 'shipping' ? '邮寄' : '线上'}</p><footer><span className="status-pill">{product.active === 0 ? '已下架' : '销售中'}</span><button className="admin-secondary" onClick={() => setEditingProduct({ ...product, description_images: product.description_images || [] })}>编辑</button></footer></article>; })}</div>
        {filteredProducts.length === 0 && <div className="admin-empty">没有符合条件的商品</div>}
        {editingProduct && <><ProductDescriptionImages product={editingProduct} images={images} setProduct={setEditingProduct} onUpload={uploadProductImage} busy={busy} /><ProductEditor product={editingProduct} categories={categories} images={images} setProduct={setEditingProduct} onUpload={uploadProductImage} onSubmit={saveProduct} onClose={() => setEditingProduct(null)} busy={busy} /></>}
      </>}
...
function ProductDescriptionImages({ product, images, setProduct, onUpload, busy }: { product: Product; images: ProductImage[]; setProduct: (product: Product) => void; onUpload: (file: File) => Promise<ProductImage | null>; busy: boolean }) {
  const [libraryOpen, setLibraryOpen] = useState(false);
  const selected = product.description_images || [];
  const addImage = (image: ProductImage) => {
    if (!product.image_id) setProduct({ ...product, image_id: image.id, image_url: image.url });
    else if (image.id !== product.image_id && !selected.some(item => item.id === image.id)) setProduct({ ...product, description_images: [...selected, image] });
  };
  return <section className="admin-panel description-image-editor"><div className="settings-heading"><Images size={22} /><div><h3>商品描述图像</h3><p>商品头像之外的图片会显示在详情页，并支持顾客放大和保存。</p></div></div><div className="product-image-actions"><label className="admin-secondary"><Upload size={15} /> 上传描述图<input type="file" accept="image/jpeg,image/png,image/webp" disabled={busy} onChange={async event => { const file = event.target.files?.[0]; if (!file) return; const image = await onUpload(file); if (image) addImage(image); event.target.value = ''; }} /></label><button type="button" className="admin-secondary" onClick={() => setLibraryOpen(value => !value)}><Images size={15} /> 从图片库选择</button></div>{selected.length > 0 && <div className="selected-description-images">{selected.map(image => <article key={image.id}><img src={image.url} alt={image.file_name} /><button type="button" aria-label={`移除 ${image.file_name}`} onClick={() => setProduct({ ...product, description_images: selected.filter(item => item.id !== image.id) })}><Trash2 size={15} /></button></article>)}</div>}{libraryOpen && <div className="image-picker">{images.map(image => <button type="button" key={image.id} disabled={image.id === product.image_id} className={selected.some(item => item.id === image.id) ? 'selected' : ''} onClick={() => addImage(image)}><img src={image.url} alt={image.file_name} /><span>{image.file_name}</span></button>)}</div>}</section>;
}
