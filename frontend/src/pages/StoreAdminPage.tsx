...
import { ExternalLink, LogOut, PackagePlus, RefreshCw, Save, ShieldCheck } from 'lucide-react';
...
export default function StoreAdminPage() {
  ...
  const createCategory = async (event: React.FormEvent) => {
    event.preventDefault(); setBusy(true); setError('');
    try { await storeApi.createCategory(newCategory); setNewCategory({ name: '', slug: '', sortOrder: 0 }); await loadData(); }
    catch (err) { setError((err as Error).message); setBusy(false); }
  };
  ...
  return <div className="store-admin admin-shell">
    <aside className="admin-sidebar"><h1>Silas Store 管理</h1><nav><button className={tab === 'orders' ? 'active' : ''} onClick={() => setTab('orders')}>订单</button><button className={tab === 'products' ? 'active' : ''} onClick={() => setTab('products')}>商品</button><button className={tab === 'categories' ? 'active' : ''} onClick={() => setTab('categories')}>分类</button></nav><button className="logout" onClick={logout}><LogOut size={15} /> 退出登录</button></aside>
    <main className="admin-main">
      <header className="admin-header"><div><small>管理员后台</small><h2>{tab === 'orders' ? '订单管理' : tab === 'products' ? '商品管理' : '分类管理'}</h2></div><div className="admin-toolbar">{tab === 'orders' && <input value={search} onChange={event => setSearch(event.target.value)} placeholder="搜索订单" />}<button className="admin-secondary" onClick={loadData} disabled={busy}><RefreshCw size={15} /> 刷新</button>{tab === 'products' && <button className="store-primary" onClick={() => setEditingProduct({ ...emptyProduct, category_id: categories[0]?.id || '' })}><PackagePlus size={15} /> 新增商品</button>}</div></header>
